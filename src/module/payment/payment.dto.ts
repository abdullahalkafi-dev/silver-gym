...
const queryPaymentDto = z.object({
  query: z
    .object({
      ...
      minAmount: z.string().regex(/^\d+(\.\d+)?$/, "Must be a valid number").optional(),
      maxAmount: z.string().regex(/^\d+(\.\d+)?$/, "Must be a valid number").optional(),
      year: z.string().regex(/^\d{4}$/, "Must be a 4-digit year").optional(),
      sort: z.string().trim().optional(),
      page: z.string().regex(/^\d+$/, "Must be a positive number").optional(),
